...
for nodes in `cat config/nodes.dat`
	...
	echo $nodes
	cont=$((cont+1))
	#ssh $username@$nodes $homedir/config/runCluster.dat $cont $homedir >>run/node$cont/run.log&
	bash $homedir/config/runClasses.dat $cont $homedir >>run/node$cont/run.log&
	done	
